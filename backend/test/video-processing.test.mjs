...
async function uploadFile(server, filename, type, bytes) {
    const form = new FormData();
    form.append("photo", new Blob([bytes], { type }), filename);
    return fetch(`${server.baseUrl}/api/upload`, { method: "POST", body: form });
}
...
describe("video upload sa zaustavljenim workerom", () => {
    ...
    test("image upload tok ostaje sinhron i ne kreira processing job", async () => {
        ...
        const response = await uploadFile(server, "photo.png", "image/png", png);
        assert.equal(response.status, 200);
