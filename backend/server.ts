...
import { randomUUID } from "crypto";         // Kriptografski bezbedna imena uploadovanih fajlova
...
import { NonRetryableJobError, ProcessingQueue, migrateProcessingJobs, type ProcessingJob } from "./processing-queue.js";
import {
...
import { probeVideoInput, VideoValidationError } from "./video-validation.js";
import {
    cleanupFfmpegPartials,
    runWatchedFfmpeg
} from "./ffmpeg-watchdog.js";
import { registerChildProcess, terminateActiveChildProcesses } from "./child-process-registry.js";
import {
    AdmissionController,
    CapacityAdmissionError,
    DiskCapacityProvider,
    type MediaAdmissionType
} from "./capacity.js";
import {
    configureSqliteReliability,
    isTransientSqliteError,
    logReliabilityError,
    SQLITE_BUSY_RETRY_AFTER_SECONDS
} from "./reliability.js";
import { streamFileToResponse } from "./stream-reliability.js";
import { OperationalRejectionLogger, type OperationalCategory } from "./operational-events.js";
import {
    FORCED_CHILD_TERM_GRACE_MS,
...
// Minimalno praćenje requestova služi shutdown dijagnostici. server.close()
// ostaje autoritativan mehanizam za drain aktivnih HTTP zahteva.
...
function getEventDecisionTime(req: express.Request): number {
    if (process.env.NODE_ENV === "test" && process.env.TEST_EVENT_NOW_HEADER === "1") {
        const testTimestamp = Date.parse(req.get("x-test-event-now") || "");
        if (Number.isFinite(testTimestamp)) return testTimestamp;
    }
...
function requireEventUnlocked(
    ...
    if (!isEventUnlocked(getEventDecisionTime(req))) {
...
function sendPendingReviewEmail(photoId: number, filename: string) {
    if (process.env.NODE_ENV === "test") {
        if (process.env.TEST_EMAIL_FAILURE === "1") {
            logReliabilityError("EMAIL_NOTIFICATION_FAILED", { notification: "pending_review", mediaId: photoId }, { code: "TEST_EMAIL_FAILURE" });
        }
        return;
    }
...
`;
    ...
    const mailProcess = execFile("mail", ["-s", subject, ADMIN_EMAIL], (error) => {
        if (error) {
            logReliabilityError("EMAIL_NOTIFICATION_FAILED", { notification: "pending_review", mediaId: photoId }, error);
            return;
        }

        console.log("Email notifikacija poslata za pending_review:", photoId);
    });
    registerChildProcess(mailProcess);

    // Telo emaila se piše direktno u standard input procesa "mail"
    mailProcess.stdin?.once("error", (error) => {
        logReliabilityError("EMAIL_NOTIFICATION_STDIN_FAILED", { notification: "pending_review", mediaId: photoId }, error);
    });
...
if (process.env.NODE_ENV === "production" && configuredOrigins.length === 0) {
    throw new Error("ALLOWED_ORIGINS mora biti definisan u production okruženju.");
}
...
class SQLiteSessionStore extends session.Store {
    ...
    constructor(filename: string) {
        ...
        const busyTimeout = process.env.NODE_ENV === "test" && process.env.TEST_DB_BUSY_TIMEOUT_MS
            ? Number(process.env.TEST_DB_BUSY_TIMEOUT_MS)
            : 5_000;
        configureSqliteReliability(this.sessionDb, busyTimeout);
        this.sessionDb.exec(`
...
const testCapacityAvailableBytes = process.env.NODE_ENV === "test"
    ? Number(process.env.TEST_CAPACITY_AVAILABLE_BYTES ?? 200 * 1024 ** 3)
    : NaN;
const testCapacityTotalBytes = process.env.NODE_ENV === "test"
    ? Number(process.env.TEST_CAPACITY_TOTAL_BYTES ?? 500 * 1024 ** 3)
    : NaN;
function createCapacityProvider(storagePath: string) {
    return Number.isFinite(testCapacityAvailableBytes) && Number.isFinite(testCapacityTotalBytes)
        ? new DiskCapacityProvider(
        storagePath,
        (async () => ({
            bsize: 1n,
            blocks: BigInt(Math.trunc(testCapacityTotalBytes)),
            bfree: BigInt(Math.trunc(testCapacityAvailableBytes)),
            bavail: BigInt(Math.trunc(testCapacityAvailableBytes)),
            files: 1n,
            ffree: 1n,
            type: 0n
        })) as unknown as typeof import("node:fs/promises").statfs
    ) : new DiskCapacityProvider(storagePath);
}

// Public upload/output i private voice storage mogu biti na različitim mount-ovima;
// svaki admission zato meri filesystem na kome će konkretan media tip stvarno pisati.
const diskCapacityProvider = createCapacityProvider(uploadsRoot);
const voiceDiskCapacityProvider = createCapacityProvider(voiceStorage.root);
...
const dbBusyTimeout = process.env.NODE_ENV === "test" && process.env.TEST_DB_BUSY_TIMEOUT_MS
    ? Number(process.env.TEST_DB_BUSY_TIMEOUT_MS)
    : 5_000;
configureSqliteReliability(db, dbBusyTimeout);
...
// Gallery/read-path indeksi nastaju tek pošto su sve legacy photos kolone
// potvrđene. Non-unique su namerno: postojeće URL/query semantike se ne menjaju.
// IF NOT EXISTS čini startup bezbednim za empty, existing i već indeksiranu DB.
db.exec(`
    CREATE INDEX IF NOT EXISTS idx_photos_status_uploaded
    ON photos(status, uploaded_at DESC, id DESC);

    CREATE INDEX IF NOT EXISTS idx_photos_original_url
    ON photos(original_url);

    CREATE INDEX IF NOT EXISTS idx_photos_thumb_url
    ON photos(thumb_url);

    CREATE INDEX IF NOT EXISTS idx_photos_web_url
    ON photos(web_url);
`);
...
// ============================================================
type AuthenticatedUser = { id: number; username: string; role: string };

function getAuthenticatedUser(req: express.Request): AuthenticatedUser | undefined {
    if (!req.session.adminId) return undefined;

    return db.prepare("SELECT id, username, role FROM admins WHERE id = ?")
        .get(req.session.adminId) as AuthenticatedUser | undefined;
}
...
app.use("/uploads", (req, res, next) => {
    ...
    // Odobreni media fajlovi ostaju javni bez session/DB provere.
...
function imageExtension(file: Express.Multer.File): string | null {
    const ext = path.extname(file.originalname).toLowerCase();
    const expectedMime = imageMimeByExtension.get(ext);
    if (!expectedMime) return null;
    if (ext === ".heic" || ext === ".heif") {
        return [expectedMime, "image/heic", "image/heif", "application/octet-stream"].includes(file.mimetype)
            ? ext
            : null;
    }
    return file.mimetype === expectedMime ? ext : null;
}

function videoExtension(file: Express.Multer.File): string | null {
    const ext = path.extname(file.originalname).toLowerCase();
    return videoExtensions.has(ext) && file.mimetype.startsWith("video/") ? ext : null;
}

function categoryStorage(destination: string, extensionFor: (file: Express.Multer.File) => string | null) {
    return multer.diskStorage({
        destination: (_req, _file, callback) => callback(null, destination),
        filename: (_req, file, callback) => {
            const ext = extensionFor(file);
            if (!ext) {
                callback(new Error("Upload type was not validated."), "");
                return;
            }
...
function imageFileFilter(legacy: boolean): NonNullable<multer.Options["fileFilter"]> {
    return (_req, file, callback) => {
        if (imageExtension(file)) {
            callback(null, true);
            return;
        }
        if (legacy && videoExtension(file)) {
            callback(new UploadRequestError(
                415,
                "VIDEO_USE_VIDEO_ENDPOINT",
                "Video fajl mora biti poslat na /api/upload/video."
            ));
            return;
        }
        callback(new UploadRequestError(415, "UNSUPPORTED_IMAGE_TYPE", "Tip slike nije podržan."));
    };
}

const videoFileFilter: NonNullable<multer.Options["fileFilter"]> = (_req, file, callback) => {
    if (videoExtension(file)) {
        callback(null, true);
        return;
    }
    callback(new UploadRequestError(415, "UNSUPPORTED_VIDEO_TYPE", "Tip video fajla nije podržan."));
};

function createImageUpload(legacy = false) {
    return multer({
        storage: categoryStorage(uploadFolderOriginal, imageExtension),
        fileFilter: imageFileFilter(legacy),
        // +1 omogućava aplikacionom gate-u da razlikuje tačno-limit od over-limit fajla.
        limits: { fileSize: IMAGE_UPLOAD_LIMIT + 1, files: 1 }
    });
}

const imageUpload = createImageUpload();
const legacyImageUpload = createImageUpload(true);
const videoUpload = multer({
    storage: categoryStorage(uploadFolderVideosOriginal, videoExtension),
    fileFilter: videoFileFilter,
    limits: { fileSize: VIDEO_UPLOAD_LIMIT + 1, files: 1 }
});

function singleMediaUpload(uploader: multer.Multer, category: "image" | "video") {
    return (req: express.Request, res: express.Response, next: express.NextFunction) => {
        uploader.single("photo")(req, res, (error: unknown) => {
            if (!error) {
                const limit = category === "image" ? IMAGE_UPLOAD_LIMIT : VIDEO_UPLOAD_LIMIT;
                if (req.file && req.file.size > limit) {
                    try {
                        if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
                    } catch (cleanupError) {
                        console.error("Oversize upload nije očišćen:", cleanupError);
                    }
                    const isImage = category === "image";
                    return res.status(413).json({
                        error: isImage ? "Slika je veća od 30 MiB." : "Video je veći od 1 GiB.",
                        code: isImage ? "IMAGE_TOO_LARGE" : "VIDEO_TOO_LARGE"
                    });
                }
                next();
                return;
...
function sendVoiceNotification(voiceId: number, senderName: string | null, durationSeconds: number): void {
    if (process.env.NODE_ENV === "test") {
        if (process.env.TEST_EMAIL_FAILURE === "1") {
            logReliabilityError("EMAIL_NOTIFICATION_FAILED", { notification: "voice", mediaId: voiceId }, { code: "TEST_EMAIL_FAILURE" });
            return;
        }
    ...
    const mailProcess = execFile("mail", ["-s", subject, COUPLE_NOTIFICATION_EMAIL], (error) => {
        if (error) {
            logReliabilityError("EMAIL_NOTIFICATION_FAILED", { notification: "voice", mediaId: voiceId }, error);
            return;
        }
        try {
            db.prepare("UPDATE voice_messages SET notification_sent_at = ? WHERE id = ?")
            ...
            logReliabilityError("EMAIL_NOTIFICATION_DB_UPDATE_FAILED", { notification: "voice", mediaId: voiceId }, dbError);
        }
    });
    registerChildProcess(mailProcess);
    mailProcess.stdin?.once("error", (error) => {
        logReliabilityError("EMAIL_NOTIFICATION_STDIN_FAILED", { notification: "voice", mediaId: voiceId }, error);
    });
    mailProcess.stdin?.write(body);
    mailProcess.stdin?.end();
}
...
function runAiModerationQueued(filePath: string) {
    if (process.env.NODE_ENV === "test" && process.env.TEST_SKIP_AI_MODERATION === "1") {
        const delayMs = Number(process.env.TEST_AI_MODERATION_DELAY_MS) || 0;
        return new Promise<{ status: string; aiScore: number; aiReason: string }>((resolve) => setTimeout(() => resolve({
                status: "approved",
                aiScore: 0,
...
async function processVideoJob(job: ProcessingJob) {
    ...
    try {
        if (!fs.existsSync(filePath)) {
            console.error(`Processing queue: missing original job=${job.id} media=${photoId} basename=${path.basename(filename)} non-retryable`);
            throw new NonRetryableJobError(`Originalni video fajl ne postoji: media=${photoId} basename=${path.basename(filename)}`);
        }

        // Metadata se ponovo čita za persistent/legacy retry poslove jer schema ostaje nepromenjena.
        const inputMetadata = await probeVideoInput(filePath);
        ...
        try {
            await runWatchedFfmpeg(thumbnailArgs(true), { jobId: job.id, mediaId: photoId }, inputMetadata);
        ...
        if (!thumbnailExists()) {
            ...
            await runWatchedFfmpeg(thumbnailArgs(false), { jobId: job.id, mediaId: photoId }, inputMetadata);
        }
        ...
        await runWatchedFfmpeg([
            ...
            "mp4",
            webVideoProcessingPath
        ], { jobId: job.id, mediaId: photoId }, inputMetadata);
    ...
    `
        ...
        try {
            cleanupFfmpegPartials([videoThumbProcessingPath, webVideoProcessingPath]);
        } catch (cleanupError) {
            logReliabilityError("VIDEO_PARTIAL_CLEANUP_FAILED", { jobId: job.id, mediaId: photoId, basename: path.basename(filename) }, cleanupError);
        }
        ...
        if (error instanceof VideoValidationError) {
            throw new NonRetryableJobError(`Video input više nije validan: media=${photoId} code=${error.code}`, { cause: error });
        }
        throw error;
    }
}
    ...
    try {
        ...
        if (fs.existsSync(finalPath)) {
            ...
            if (!fs.existsSync(sourcePath)) {
                console.error(`Processing queue: missing original job=${job.id} media=${voice.id} basename=${path.basename(voice.sourceFilename)} non-retryable`);
                throw new NonRetryableJobError(`Originalna glasovna poruka ne postoji: media=${voice.id} basename=${path.basename(voice.sourceFilename)}`);
            }
        ...
        if (fs.existsSync(processingPath)) {
            try { fs.unlinkSync(processingPath); } catch (cleanupError) {
                logReliabilityError("VOICE_PARTIAL_CLEANUP_FAILED", { jobId: job.id, mediaId: voice.id, basename: path.basename(processingPath) }, cleanupError);
            }
        }
        throw error;
    }
...
let processingClaimPauseState: string | null = null;
const processingQueue = new ProcessingQueue({
    db,
    beforeClaim: async () => {
        if (lifecycle.isShuttingDown) return false;
        const capacities = await Promise.all([diskCapacityProvider.get(), voiceDiskCapacityProvider.get()]);
        const blockingState = capacities.find((capacity) => capacity.state === "emergency" || capacity.state === "unknown")?.state;
        if (blockingState) {
            if (processingClaimPauseState !== blockingState) {
                console.warn(`Processing queue: claim pauziran zbog disk state=${blockingState}`);
                processingClaimPauseState = blockingState;
            }
            return false;
        }
        if (processingClaimPauseState) {
            console.log("Processing queue: disk capacity obnovljen; claim nastavljen");
            processingClaimPauseState = null;
        }
        return true;
    },
...
const admissionController = new AdmissionController(
    (type) => type === "voice" ? voiceDiskCapacityProvider : diskCapacityProvider,
    () => processingQueue.stats("video_process")
);
const operationalRejections = new OperationalRejectionLogger();

function operationalCategoryForPath(requestPath: string): OperationalCategory {
    if (requestPath === "/api/upload/video") return "video";
    if (requestPath === "/api/upload/image" || requestPath === "/api/upload") return "image";
    if (requestPath === "/api/voice-messages") return "voice";
    return "api";
}

function capacityAdmission(type: MediaAdmissionType) {
    return async (req: express.Request, res: express.Response, next: express.NextFunction) => {
        if (lifecycle.isShuttingDown) {
            operationalRejections.record("CAPACITY_REJECTION", type, "SERVICE_SHUTTING_DOWN");
            res.setHeader("Retry-After", String(SHUTDOWN_RETRY_AFTER_SECONDS));
            return res.status(503).json({
                error: "Servis se trenutno kontrolisano zaustavlja. Pokušajte ponovo uskoro.",
                code: "SERVICE_SHUTTING_DOWN"
            });
        }
        const abortController = new AbortController();
        const abortPending = () => abortController.abort();
        req.once("aborted", abortPending);
        try {
            const release = await admissionController.acquire(type, abortController.signal);
            req.off("aborted", abortPending);
            let released = false;
            const releaseOnce = () => {
                if (released) return;
                released = true;
            ...
            if (error instanceof CapacityAdmissionError) {
                operationalRejections.record("CAPACITY_REJECTION", type, error.code);
                res.setHeader("Retry-After", String(error.retryAfterSeconds));
                return res.status(error.status).json({
                    error: "Server trenutno obrađuje veći broj uploadova. Pokušajte ponovo za nekoliko minuta.",
                    code: error.code
                });
            }
            next(error);
...
// ============================================================
// Zajednička obrada za strict image i legacy image-only endpoint.
// ============================================================
async function unlinkUploadInput(filePath: string) {
    for (let attempt = 0; attempt < 6; attempt++) {
        try {
            ...
            if ((code !== "EBUSY" && code !== "EPERM") || attempt === 5) throw error;
            await new Promise((resolve) => setTimeout(resolve, 50 * (attempt + 1)));
        }
    }
}

async function handleImageUpload(req: express.Request, res: express.Response) {
    const file = req.file;
    ...
    try {
        ...
        if (isVideo) {
            ...
            // libvips can retain WebP/GIF file handles briefly on Windows. Feeding
            // Sharp a bounded in-memory buffer prevents the accepted upload from
            // failing later when the temporary source file is removed.
            const sharpInput: string | Buffer = process.platform === "win32"
                ? await fs.promises.readFile(sharpInputPath)
                : sharpInputPath;

            let metadata: sharp.Metadata;
            try {
                metadata = await sharp(sharpInput, {
                ...
                if (message.includes("pixel limit")) {
                    throw new UploadRequestError(
                        ...
                        "Slika prelazi dozvoljenih 100 megapiksela."
                    );
                }
                throw new UploadRequestError(422, "INVALID_IMAGE", "Fajl nije validna slika.");
            }

            const width = metadata.width;
            const height = metadata.pageHeight ?? metadata.height;
            const expectedFormat = isHeic ? "jpeg" : ext === ".jpg" || ext === ".jpeg" ? "jpeg" : ext.slice(1);
            if (!width || !height || metadata.format !== expectedFormat) {
                throw new UploadRequestError(422, "INVALID_IMAGE", "Sadržaj ne odgovara prijavljenom tipu slike.");
            }
            if (imageDimensionViolation(width, height)) {
                throw new UploadRequestError(
                    422,
                    "IMAGE_DIMENSIONS_EXCEEDED",
                    "Dimenzije slike prelaze dozvoljeni maksimum."
                );
            }

            // Puna dekodiranja koriste isti pixel limit i failOn=error za corrupt/truncated input.
            ...
            await unlinkUploadInput(file.path);
            if (convertedHeicPath && fs.existsSync(convertedHeicPath)) {
                await unlinkUploadInput(convertedHeicPath);
...
app.post(
    "/api/upload/image",
    requireEventUnlocked,
    capacityAdmission("image"),
    uploadLimiter,
    singleMediaUpload(imageUpload, "image"),
    handleImageUpload
);
app.post(
    "/api/upload",
    ...
    capacityAdmission("image"),
    uploadLimiter,
    singleMediaUpload(legacyImageUpload, "image"),
    handleImageUpload
);

// Video endpoint zadržava postojeći DB + processing queue tok; duboka validacija pripada Phase 1C.
app.post(
    "/api/upload/video",
    requireEventUnlocked,
    capacityAdmission("video"),
    uploadLimiter,
    singleMediaUpload(videoUpload, "video"),
    async (req, res) => {
        const file = req.file;
        if (!file) return res.status(400).json({ error: "Nijedan fajl nije poslat." });

        try {
            if (!(process.env.NODE_ENV === "test" && process.env.TEST_VIDEO_PROBE === "stub")) {
                await probeVideoInput(file.path);
            }
            const status = "pending_review";
            const originalUrl = `/uploads/videos/original/${file.filename}`;
            const insertMedia = db.transaction(() => {
                const result = db.prepare(`
                    INSERT INTO photos (
                        filename, original_url, thumb_url, status, uploaded_at,
                        ai_score, ai_reason, media_type, web_url
                    ) VALUES (?, ?, '', ?, ?, 0, ?, 'video', '')
                `).run(
                    file.filename,
                    originalUrl,
                    status,
                    new Date().toISOString(),
                    "Video fajl - obrada u toku, ručni pregled potreban"
                );
            ...
            return res.json({
                message: "Video je uploadovan i obrada je pokrenuta u pozadini.",
            ...
            try {
                ...
                console.error("Greška pri brisanju neuspelog video fajla:", cleanupError);
            }
            if (error instanceof VideoValidationError) {
                console.warn(`Video input validation odbijen: file=${path.basename(file.path)} code=${error.code}`);
                return res.status(422).json({
                    error: "Video fajl nije validan ili podržan.",
                    code: error.code
                });
            }
            console.error("Greška pri prihvatanju video fajla:", error);
            return res.status(500).json({ error: "Greška pri obradi fajla." });
        }
    }
);
...
// ============================================================
app.get("/api/photos/:id/download", (req, res, next) => {
...
`
    ...
    res.download(filePath, photo.filename, (error) => {
        if (!error) return;
        logReliabilityError("MEDIA_DOWNLOAD_FAILED", { mediaId: id, basename: path.basename(photo.filename) }, error);
...
// ============================================================
app.post("/api/photos/:id/like", likeLimiter, (req, res, next) => {
    const id = parsePositiveId(req.params.id);
    if (id === null) {
        return res.status(400).json({ error: "Neispravan ID." });
    }
    ...
    try {
        ...
        if (!isDuplicateLike) {
            if (isTransientSqliteError(error)) return next(error);
...
app.get("/api/couple/voice-messages/:id/stream", requireCouple, (req, res) => {
    ...
    if (!range) {
        ...
        streamFileToResponse(filePath, res, {
            operation: "voice-stream",
            mediaId: voice.id,
            basename: path.basename(voice.filename)
        });
        return;
    }
    ...
    streamFileToResponse(filePath, res, {
        ...
        operation: "voice-range-stream",
        mediaId: voice.id,
        basename: path.basename(voice.filename)
    });
});

app.get("/api/couple/voice-messages/:id/download", requireCouple, (req, res, next) => {
    const id = parsePositiveId(req.params.id);
    if (id === null) return res.status(400).json({ error: "Neispravan ID." });
    const voice = getReadyVoice(id);
    if (!voice) return res.status(404).json({ error: "Glasovna poruka nije spremna." });
    ...
    res.download(filePath, `${date}_${safeDownloadSender(voice.senderName)}_${voice.id}.m4a`, (error) => {
        if (!error) return;
        logReliabilityError("VOICE_DOWNLOAD_FAILED", { mediaId: voice.id, basename: path.basename(voice.filename) }, error);
        if (res.headersSent) res.destroy(error);
        else next(error);
    });
});
...
// Readiness je odvojen od liveness-a: kratkotrajno pun upload pipeline ili
// reject disk prag ne isključuju read-only gallery servis. Shutdown, emergency
// i nepoznato disk stanje znače da instanca nije spremna za novi posao.
app.get("/api/ready", async (req, res) => {
    try {
        const capacities = await Promise.all([diskCapacityProvider.get(), voiceDiskCapacityProvider.get()]);
        const decision = readinessDecision(lifecycle, capacities.map((capacity) => capacity.state));
        if (!decision.ready) {
            return res.status(503).json({ status: "not_ready", code: "SERVICE_NOT_READY" });
        }
        return res.json({ status: "ready" });
    } catch (error) {
        logReliabilityError("READINESS_CHECK_FAILED", {}, error);
        return res.status(503).json({ status: "not_ready", code: "SERVICE_NOT_READY" });
    }
});

if (process.env.NODE_ENV === "test" && process.env.TEST_SLOW_REQUEST_MS) {
    app.get("/api/test/slow", async (req, res) => {
        await new Promise((resolve) => setTimeout(resolve, Number(process.env.TEST_SLOW_REQUEST_MS)));
...
// Serve the public frontend from the same origin as the API. This keeps local
// setup simple and avoids weakening CORS or cookie protections for development.
app.use(express.static(path.join(projectRoot, "frontend")));

app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
    logReliabilityError("REQUEST_ERROR", { method: req.method, route: req.path }, err);

    if (isTransientSqliteError(err)) {
        operationalRejections.record(
            "DB_CONTENTION_REJECTION",
            operationalCategoryForPath(req.path),
            "DB_TEMPORARILY_BUSY"
        );
        res.setHeader("Retry-After", String(SQLITE_BUSY_RETRY_AFTER_SECONDS));
        return res.status(503).json({
            error: "Baza je privremeno zauzeta. Pokušajte ponovo.",
...
async function gracefulShutdown(signal: string) {
    if (!lifecycle.beginShutdown()) {
        console.warn(`${signal} primljen dok je shutdown već u toku; dupli cleanup nije pokrenut.`);
        return;
    }

    console.log(`${signal} primljen; bounded graceful shutdown počinje.`);
    const configuredTimeout = process.env.NODE_ENV === "test"
        ? Number(process.env.TEST_SHUTDOWN_TIMEOUT_MS ?? GRACEFUL_SHUTDOWN_TIMEOUT_MS)
        : GRACEFUL_SHUTDOWN_TIMEOUT_MS;
    const shutdownTimeoutMs = Number.isFinite(configuredTimeout) && configuredTimeout > 0
        ? configuredTimeout
        : GRACEFUL_SHUTDOWN_TIMEOUT_MS;

    // stop() odmah sprečava sledeći claim, ali dozvoljava aktivnom poslu da završi.
    const queueDrain = processingQueue.stop();
    const listenerClosed = new Promise<void>((resolve, reject) => {
        server.close((error) => error ? reject(error) : resolve());
        server.closeIdleConnections?.();
    });
    // Neki keep-alive klijenti mogu zadržati close callback i nakon što su svi
    // requestovi završeni. Aktivni request counter daje determinističan drain;
    // idle konekcije se tada zatvaraju ponovo, bez prekidanja aktivnog requesta.
    const requestsDrained = lifecycle.waitForRequestsToDrain().then(() => {
        server.closeIdleConnections?.();
    });
    const httpDrain = Promise.race([listenerClosed, requestsDrained]);
    const logDrainFailure = async (step: string, drain: Promise<unknown>) => {
        try {
            await drain;
        } catch (error) {
            logReliabilityError("SHUTDOWN_DRAIN_STEP_FAILED", { step }, error);
        }
    };
    const applicationDrain = Promise.all([
        logDrainFailure("http", httpDrain),
        logDrainFailure("processing-queue", queueDrain),
        logDrainFailure("image-moderation", aiQueue)
    ]);
    const drained = await settleWithin(applicationDrain, shutdownTimeoutMs);

    if (!drained.completed) {
        console.error(
            `Shutdown deadline istekao posle ${shutdownTimeoutMs}ms; ` +
            `activeRequests=${lifecycle.activeRequests}. Pokreće se bounded forced cleanup.`
        );
        server.closeAllConnections?.();
        try {
            const result = await terminateActiveChildProcesses(FORCED_CHILD_TERM_GRACE_MS);
            if (result.terminated > 0) {
                console.warn(
                    `Shutdown child cleanup: TERM=${result.terminated} ` +
                    `KILL=${result.killed} remaining=${result.remaining}`
                );
            }
        } catch (error) {
            logReliabilityError("SHUTDOWN_CHILD_CLEANUP_FAILED", { step: "child-processes" }, error);
        }
        // Ne zatvaramo DB ispod još aktivnog job-a. Process exit zatvara descriptor,
        // a postojeći stale recovery vraća eventualni processing job nakon restarta.
        process.exit(1);
        return;
    }

    for (const [step, close] of [
        ["session", () => sessionStore.close()],
        ["database", () => db.close()]
    ] as const) {
        try {
            close();
        } catch (error) {
            logReliabilityError("SHUTDOWN_RESOURCE_CLOSE_FAILED", { step }, error);
        }
    }
    console.log("Graceful shutdown završen.");
    process.exit(0);
}
